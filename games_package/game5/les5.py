...
def guess(min_number, max_number, max_attempts):
    # Генерируем случайное число, которое нужно угадать, в заданном диапазоне.
    ...
    while attempts_left > 0:
        # Запрашиваем у пользователя ввод числа и отображаем оставшееся количество попыток.
        user_guess = input(f"Угадайте число от {min_number} до {max_number}. У вас {attempts_left} попыток: ")
        
        try:
            user_guess = int(user_guess)  # Преобразуем введенную строку в целое число.
        except ValueError:
            print("Введите целое число.")
            continue  # Пропускаем текущую итерацию цикла, если введенное значение не является числом.
        ...
        attempts_left -= 1  # Уменьшаем количество оставшихся попыток после каждой попытки.

    if attempts_left == 0:
        # Выводим сообщение, если пользователь использовал все попытки и не угадал число.
        print(f"Вы использовали все попытки. Загаданное число было {secret_number}.")

# Пример вызова декорированной функции.
results = guess(1, 100, 5)
